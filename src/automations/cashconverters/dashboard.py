...
def _card(listing) -> str:
    ...
    return f'''<article class="product {'offer' if reasons else ''}" data-search="{_e(search)}"
      ...
      <p class="store">{_e(listing.store or 'Magasin inconnu')}</p>{reason}
...
def build_html(repo) -> str:
    ...
    last_new = int(state.get('new', 0) or 0)
...
.sub,.muted,.store,.specs{{color:var(--muted)}}.health{{text-align:right}}.health strong{{display:inline-block;padding:7px 10px;border-radius:99px;background:#1b2a43}}.health .ok{{color:var(--green)}}.health .bad{{color:var(--hot)}}
...
.view{{display:none;padding-top:22px}}.view.active{{display:block}}.kpis{{display:grid;grid-template-columns:repeat(6,1fr);gap:12px}}.kpi{{background:linear-gradient(145deg,var(--panel2),var(--panel));border:1px solid var(--line);padding:16px;border-radius:15px}}.kpi span{{color:var(--muted);font-size:12px}}.kpi strong{{display:block;font-size:28px;margin-top:5px}}.kpi.hot strong{{color:var(--hot)}}
...
<section class="view active" id="overview">
 ...
 <div class="kpi"><span>Nouveaux au dernier scan</span><strong>{last_new}</strong></div></div>
